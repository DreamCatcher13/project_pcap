...
while (user_choice != '7'):
	...
	if user_choice == '1':
		...
		if ip1 not in ip_list or ip2 not in ip_list:
			...
			(result, packets) = session(pkts, ip1, ip2)
			print('\n' + result + '\n')
			print('Do you want to save the result in txt file? yes/no')
			s = input('Answer: ')
			if s == 'yes':
